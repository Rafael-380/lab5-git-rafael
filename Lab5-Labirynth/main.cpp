...
int main() {
    // Criar a janela
    sf::RenderWindow window(sf::VideoMode(800, 600), "Labyrinth - Step 1");

    // Carregar textura de relva
    sf::Texture grassTexture;
    if (!grassTexture.loadFromFile("grass.png")) {
        std::cerr << "Erro ao carregar grass.png" << std::endl;
        return 1;
    }
    grassTexture.setRepeated(true); // permite tiling da textura

    // Criar o sprite de fundo
    sf::Sprite background;
    background.setTexture(grassTexture);

    // Obter tamanho da janela e definir o rectângulo da textura para o fundo
